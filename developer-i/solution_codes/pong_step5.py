...
#   ครั้งนี้เราได้ลองชิม game AI ครั้งแรก คือ "กฎที่อ่านสถานะแล้วคืนการเคลื่อนไหว"
#
# เรื่องราวของ step นี้: ครั้งนี้เป็นเกม Pong เต็มตัวแล้ว มีหน้าเลือกโหมด EASY/NORMAL/HARD
#   ก่อนเริ่ม (เลือกด้วย UP/DOWN, A เริ่ม), แต่ละโหมดปรับ "ความเก่งของ AI" และความเร็ว
#   ลูกกับแต้มที่ต้องชนะ หัวใจคือ AI tracker เล็ก ๆ ที่อ่านตำแหน่งลูกแล้วตัดสินใจขยับ
#   ขึ้น/ลง นี่แหละคือ "AI เกม" ตัวแรกของคุณ
#
# ใหม่จาก step4: แทนไม้ขวาที่เป็นผู้เล่น ด้วย "AI tracker" ที่วิ่งตามลูก
#   และเพิ่มตารางโหมด EASY/NORMAL/HARD (ai_speed, ball_mul, win_score)
# ส่วนที่คุณเขียนเอง: AI tracker (target_y = ball_y - PADDLE_H/2; เลื่อนเข้าหา target),
#   เลือกพารามิเตอร์จากโหมดที่เลือก
# ส่วนที่ core เตรียมไว้ให้: game.hit, game.sfx, game.Text และของเดิม
# C step กับ Python step บนจอ: page_game_pong.c (ของจริง) มีพฤติกรรม AI/สะท้อน/
#   คะแนน "ระดับ logic เหมือนกัน"; ฝั่ง C เพิ่ม chrome (menu/scanlines/music) ที่
#   Python ไม่มี (เป็น asymmetry ที่ตั้งใจ ตาม MASTER_PLAN §2.3) Python เล่นได้จริงแต่เรียบกว่า
#
# จุดอ้างอิงในโค้ดจริง: page_game_pong.c:310-316 (AI tracker), :50-54 (โหมด)
# ฝั่ง Python ที่จับคู่กัน: pong_full.py:40-44 (AI)
#
# จุดที่ตัวเลขต่างกันโดยตั้งใจ (ลองหาและจับคู่ตัวเลข แบบฝึก §1.4):
#   - Python NORMAL AI step = 3.2 (ตรงกับ C s_pong_modes[NORMAL].ai_speed) ;
#     pong_full.py ใช้ 4.2 ตรงนี้เราจัดให้ตรงกับ C ผ่านตารางโหมด
...
# ----- เติมส่วนนี้เอง (งานของคุณ): ตารางโหมด (mirror ของ s_pong_modes[] ใน C :50-54) -----
...
def on_each_frame():
    ...
    # ----- เติมส่วนนี้เอง (งานของคุณ): AI tracker (ไม้ขวา) กฎ AI ที่ง่ายที่สุด -----
    # ----- เติมส่วนนี้เอง: เล็งให้กลางไม้ตรงลูก แล้วก้าวเข้าหาทีละ ai_speed -----
