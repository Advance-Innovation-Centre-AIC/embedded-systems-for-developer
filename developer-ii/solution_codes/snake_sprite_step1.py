# snake_sprite_step1.py — คาบ 4 (ช่วง B): กล่อง -> sprite จริง (ภาพนิ่ง)
#
# ไฟล์นี้คือ snake_step1.py (วาดสนาม+ตัวงู+อาหารแบบภาพนิ่ง) แต่เปลี่ยน "วิธีวาด"
# จาก game.Box(...) เป็น game.Sprite("ชื่อรูป", x, y) เพื่อให้งูเป็น
# พิกเซลอาร์ตตัวเดียวกับเกม C เป๊ะ  (logic การเดินยังไม่มี — คาบนี้แค่เปลี่ยนตัววาด)
#
# 2 จุดที่เปลี่ยนจากเวอร์ชันกล่อง:
#   1) Box(...)  ->  Sprite("ชื่อรูป", x, y)   (หัว/ตัว/อาหาร เป็นรูปจริง)
#   2) CELL = 26 -> CELL = 16                  (ขนาดช่อง = SNAKE_CELL_PX ในเกม C)
#
# บนฮาร์ดแวร์จริง = รูปพิกเซลเป๊ะเหมือนเกม C
# บน sim บนคอม    = กล่องเขียว GB เล็ก ๆ (fallback) — logic เหมือนกันทุกอย่าง
