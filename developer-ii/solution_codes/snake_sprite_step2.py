...
# คาบนี้ใส่ "สมอง" 30% Python ครบลูป:
...
# ----- สร้าง sprite ครั้งเดียว (หัว + pool ลำตัว + อาหาร) -- ห้ามสร้างใน update() -----
...
def update():
    ...
    k = game.keys()
    if k.left and step_x == 0:  step_x, step_y = -1, 0  # เลี้ยว (กันถอยหลังทับตัว)
    if k.right and step_x == 0: step_x, step_y = 1, 0
    if k.up and step_y == 0:    step_x, step_y = 0, -1
    if k.down and step_y == 0:  step_x, step_y = 0, 1
    # (Back=ออก / Start=เริ่มใหม่ — game.run() จัดการให้)

    next_head = [body[0][0] + step_x, body[0][1] + step_y]   # ช่องถัดไปของหัว
    # ตรวจ "ตาย" ก่อน insert เสมอ: ชนกำแพง หรือ หัวไปอยู่ใน body (ชนตัวเอง)
    if next_head[0] < 0 or next_head[0] >= COLS or next_head[1] < 0 or next_head[1] >= ROWS or next_head in body:
        game.sfx("die")
        game.Text("GAME OVER", 330, 180, game.RED)
        return False

    body.insert(0, next_head)                  # เติมหัวใหม่เสมอ
    if next_head == food_cell:                 # กินอาหาร?
        score += 1
        game.sfx("eat")
        score_text.set("Score: %d" % score)
        place_food()
        if len(body) > MAX_BODY:              # pool เต็ม -> หยุดโต (เพดาน 32 widget จริงของบอร์ด)
            body.pop()
    else:
        body.pop()                            # ไม่กิน -> ตัดหาง (เดินปกติ)
    draw()
